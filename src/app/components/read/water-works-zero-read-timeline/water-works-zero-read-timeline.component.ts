...
@Component({
  selector: 'nal-jal-water-works-zero-read-timeline',
  templateUrl: './water-works-zero-read-timeline.component.html',
  styleUrls: ['./water-works-zero-read-timeline.component.css']
})
export class WaterWorksZeroReadTimelineComponent implements OnInit {
  ...
  getReadings(discomId, regionId, circleId, divisionId, locationCode){
    this.readService.getZeroReadingsByBillMonthAndlocation(this.billMonth, discomId, regionId, circleId, divisionId, locationCode, this.RESPONSE_FALSE).subscribe(success =>{
      this.loading = false;
