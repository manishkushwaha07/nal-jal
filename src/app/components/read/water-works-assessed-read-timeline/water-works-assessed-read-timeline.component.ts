...
export class WaterWorksAssessedReadTimelineComponent implements OnInit {
  ...
  getReadings(discomId, regionId, circleId, divisionId, locationCode){
    this.readService.getAssessedReadingsByBillMonthAndlocation(this.billMonth, discomId, regionId, circleId, divisionId, locationCode, this.RESPONSE_FALSE).subscribe(success =>{
      this.loading = false;
